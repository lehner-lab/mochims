...
#' @export
#' @import data.table
mochims__predict_fitness <- function(
  mochi_outpath,
  dataset_name,
  folding_energy,
  binding_energy,
  ...
  #Suffices
  dataset_name_abundance <- sapply(strsplit(dataset_name, "_"), '[', 1)
  dataset_name_binding <- dataset_name

  #Linear transformation parameters
  linears_file_abundance <- file.path(mochi_outpath, "task_1", 'weights', paste0("linears_weights_Abundance", dataset_name_abundance, ".txt"))
  ...
  if(!is.null(folding_energy)){
    pred_list[["fraction_folded"]] <- mochims__fraction_folded(folding_energy, RT)
  ...
  if(!is.null(folding_energy) & !is.null(binding_energy) & length(folding_energy) == length(binding_energy)){
    #Linear transformation parameters
    linears_file_binding <- file.path(mochi_outpath, "task_1", 'weights', paste0("linears_weights_Binding", dataset_name_binding, ".txt"))
    linears_params_binding <- as.list(fread(linears_file_binding)[1,2:3])
    #Binding fitness
    pred_list[["fraction_bound"]] <- mochims__fraction_bound(folding_energy, binding_energy, RT)
