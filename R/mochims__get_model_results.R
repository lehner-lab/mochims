...
#' @param normalisation_file path to normalisation file (required)
#' @param RT constant (default:0.001987*(273+24))
#'
#' @return data.table with model results
#' @export
#' @import data.table
mochims__get_model_results <- function(
  input_folder, 
  normalisation_file, 
  RT = 0.001987*(273+24)
  ){
  ...
  if(file.exists(file.path(input_folder, task_folder, 'predictions', "predicted_phenotypes_supp.txt"))){
    supp_dt <- fread(file.path(input_folder, task_folder, 'predictions', "predicted_phenotypes_supp.txt"))
    pred_dt <- supp_dt[!aa_seq %in% pred_dt[,aa_seq]]
  }
  #Mutation order
  if("Nham_aa" %in% names(pred_dt)){
    pred_dt[, mut_order := Nham_aa]
  }else{
    pred_dt[, mut_order := Nham_nt]
  }
  #Predicted phenotype for held out test data
  if("Fold" %in% names(pred_dt)){
    for(i in 1:10){pred_dt[Fold==i, fitness_pred := .SD[[1]],,.SDcols = paste0('fold_', i)]}
  }else{
    pred_dt[, fitness_pred := .SD[[1]],,.SDcols = "mean"]
  }

  # #Normalise fitness data to independent measurements
  # if(normalisation_file!="" & file.exists(normalisation_file)){
  #   norm_dt <- fread(normalisation_file)
  #   names(norm_dt)[2] <- "normalisation_value"
  #   pred_dt <- merge(pred_dt, norm_dt, by = names(norm_dt)[1], all.x = T)
  #   #Normalise exponent of fitness?
  #   if(startsWith(basename(normalisation_file), "exp")){
  #     scale_factor <- pred_dt[!is.na(normalisation_value),normalisation_value/exp(fitness)]
  #     for(i in 1:10){
  #       pred_dt[, paste0("fold_", i) := scale_factor*exp(.SD[[1]]),,.SDcols = paste0("fold_", i)]
  #     }
  #     pred_dt[, fitness := scale_factor*exp(fitness)]
  #     pred_dt[, fitness_pred := scale_factor*exp(fitness_pred)]
  #   }else{
  #     scale_factor <- pred_dt[!is.na(normalisation_value),normalisation_value/fitness]
  #     for(i in 1:10){
  #       pred_dt[, paste0("fold_", i) := scale_factor*.SD[[1]],,.SDcols = paste0("fold_", i)]
  #     }
  #     pred_dt[, fitness := scale_factor*fitness]
  #     pred_dt[, fitness_pred := scale_factor*fitness_pred]
  #   }
  # }

  #Coefficients
  coef_dt <- data.table()
  for(i in dir(file.path(input_folder, task_folder, 'weights'))[grepl("^weights_(?!agg_)", dir(file.path(input_folder, task_folder, 'weights')), perl = T)]){
    coef_dt <- rbind(
      coef_dt,
      fread(file.path(input_folder, task_folder, 'weights', i), colClasses = list('character' = c('Pos', 'Pos_ref'))))
  }

  # #Model coefficients - folding
  # if(file.exists(file.path(input_folder, task_folder, 'weights', "weights_Folding.txt"))){
  #   coef_dt <- rbind(
  #     coef_dt,
  #     fread(file.path(input_folder, task_folder, 'weights', "weights_Folding.txt"), colClasses = list('character' = c('Pos', 'Pos_ref'))))
  # }
  ...
  #   #Model coefficients - folding
  #   if(file.exists(file.path(input_folder, "task_1", 'weights', "weights_Folding.txt"))){
  #     temp_dt <- rbind(
  #       temp_dt,
  #       fread(file.path(input_folder, "task_1", 'weights', "weights_Folding.txt"), colClasses = list('character' = c('Pos', 'Pos_ref'))))
  #   }
  #   #Model coefficients - binding
